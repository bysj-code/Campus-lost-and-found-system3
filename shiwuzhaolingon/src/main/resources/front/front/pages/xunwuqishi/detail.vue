				...
				<view class="yinyuewai">
					<view class="yinyuenei">
						...
						<view class="shangpinfei">
							{{detail.xunwuqishiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
	...
	export default {
		...
		methods: {
			...
			async submitData() {
				...
				let data = {
					xunwuqishiId: this.xunwuqishiId,
					yonghuId: this.user.yonghuId,
					xunwuqishiLiuyanPingfenNumber: this.scoreData,
					xunwuqishiLiuyanText: this.commentData,
				}
				await this.$api.save(`xunwuqishiLiuyan`, data);
				this.$refs.deliver.close()
				this.mescroll.num = 1
				this.upCallback(this.mescroll)
			},
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('xunwuqishi', this.xunwuqishiId);
				this.detail = res.data
				this.detail.xunwuqishiContent = this.detail.xunwuqishiContent.replace("<img", "<img  class=\"daxiao\"")
